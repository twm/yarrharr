...
from ..application import AdaptiveLoopingCall, CSPReport, Root, Static, TwistedLoggerLogHandler, formatForSystemd
...
class StaticTests(SynchronousTestCase):
    ...
    def assertResponse(self, d, *, code=200, content_type="", content_encoding="", content_length="0"):
        response = self.successResultOf(d)
        self.assertEqual(200, response.code)
        self.assertEqual([content_length], response.headers.getRawHeaders("content-length"))
        self.assertEqual(
            ["public, max-age=31536000, immutable"],
            response.headers.getRawHeaders("cache-control"),
        )
        self.assertEqual([content_type], response.headers.getRawHeaders("content-type", [""]))
        self.assertEqual([content_encoding], response.headers.getRawHeaders("content-encoding", [""]))
...
class FormatForSystemdTests(SynchronousTestCase):
    ...
    def test_logging_exception(self):
        ...
        self.assertTrue(all(ln.startswith(("<3>[", "<3>  ")) for ln in output.splitlines()))
